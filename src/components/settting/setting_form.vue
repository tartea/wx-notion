<template>
    <div class="setting-form">
        <div class="setting-form-header">
            <svg t="1718705883397" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
                p-id="5094" width="40" height="40">
                <path
                    d="M647.165837 2.332635L78.85493 44.179432C33.04873 48.138835 17.074585 78.005024 17.074585 113.810318v621.148461c0 27.886487 9.966774 51.745305 33.893858 83.625329l133.595729 173.189762c21.947382 27.886487 41.88093 33.859725 83.795993 31.880024l659.957441-39.832963c55.807107-3.959403 71.781251-29.866189 71.781251-73.658555V211.361824c0-22.630038-8.976923-29.1494-35.3957-48.468558a1594.683818 1594.683818 0 0 1-4.505528-3.276748L778.781865 32.198823C734.8871 0.387066 716.967387-3.640603 647.165837 2.332635zM283.310326 199.92734c-53.895671 3.618075-66.115209 4.437262-96.732319-20.377274l-77.822755-61.712079c-7.918807-7.987072-3.925271-17.953846 15.974144-19.933548l546.363525-39.79883c45.840333-3.993536 69.767417 11.946476 87.721263 25.872653l93.728634 67.71945c3.993536 1.979702 13.926177 13.892044 1.979702 13.892044l-564.249106 33.859725-6.963088 0.477859zM220.471864 904.189138V310.961297c0-25.906785 7.952939-37.853261 31.880024-39.867096l648.010965-37.819128c21.981515-1.979702 31.948289 11.946476 31.948289 37.819128v589.268439c0 25.906785-3.993536 47.820035-39.935361 49.799736l-620.090346 35.839427c-35.873559 1.979702-51.813571-9.932641-51.813571-41.812665z m612.171539-561.416084c3.959403 17.919713 0 35.839427-17.987979 37.887394l-29.900321 5.904972v437.991926c-25.940918 13.926177-49.833869 21.879117-69.767417 21.879116-31.914156 0-39.935361-9.966774-63.828313-39.79883l-195.444339-306.580694v296.613921l61.84861 13.96031s0 35.839427-49.902135 35.839426l-137.555132 7.95294c-3.959403-7.987072 0-27.886487 13.994443-31.845891l35.839426-9.932641v-392.185725l-49.833869-4.027669c-3.959403-17.919713 5.973238-43.792366 33.92799-45.772068l147.55604-9.966773 203.397279 310.608363v-274.768937l-51.881837-5.939105c-3.959403-21.947382 11.946476-37.853261 31.914156-39.832962l137.623398-7.987073z"
                    fill="#000000" p-id="5095"></path>
            </svg>
            <span class="form-header-title">Notion</span>
        </div>
        <el-form ref="ruleFormRef" :model="ruleForm" status-icon :rules="rules" label-width="auto"
            class="demo-ruleForm">
            <el-form-item label="同步类型" prop="pageSyncType">
                <el-select v-model="ruleForm.pageSyncType" placeholder="数据库或者页面" @change="changeSelected()">
                    <el-option label="数据库" key="database" value="database" />
                    <el-option label="页面" key="page" value="page" />
                </el-select>
            </el-form-item>
            <el-form-item label="名称" prop="pageTitle">
                <el-input v-model="ruleForm.pageTitle" type="text" autocomplete="off" />
            </el-form-item>
            <el-form-item label="Secret Key" prop="pageSecret">
                <el-input v-model="ruleForm.pageSecret" type="text" autocomplete="off" />
            </el-form-item>
            <el-form-item label="页面ID" prop="pageId">
                <el-input v-model="ruleForm.pageId" type="text" />
            </el-form-item>

            <el-form-item>
                <el-button type="primary" @click="submitForm(ruleFormRef)">
                    保存配置
                </el-button>
                <el-button @click="deleteForm()">删除</el-button>
                <el-button @click="connectPage(ruleFormRef)">获取页面ID</el-button>

                <div class="el-dropdown-button" v-show="isShowDropNotion">
                    <el-dropdown @command="handleCommand">
                        <span class="el-dropdown-link">
                            选择页面ID
                            <el-icon class="el-icon--right">
                                <arrow-down />
                            </el-icon>
                        </span>
                        <template #dropdown>
                            <el-dropdown-menu>
                                <el-dropdown-item :command="item.pageId" v-for="(item, index) in pageDropList"
                                    :key='index'> {{ item.plainTitle }}</el-dropdown-item>
                            </el-dropdown-menu>
                        </template>
                    </el-dropdown>
                </div>
            </el-form-item>
        </el-form>
    </div>
</template>

<script setup>
import { defineEmits, defineProps, reactive, ref, watchEffect } from 'vue';
import { searchNotion } from '../../http/notionApi'
import { ArrowDown } from '@element-plus/icons-vue'

const ruleFormRef = ref()
const props = defineProps(['pageTitle', 'pageSecret', 'pageId', 'uuId', 'pageSyncType'])
const emit = defineEmits(['updateConfig'])
const isShowDropNotion = ref(false)
const pageDropList = ref([])


const rules = reactive({
    pageSyncType: [
        { required: true, message: '请选择同步类型', trigger: 'blur' }
    ],
    pageTitle: [
        { required: true, message: '请填写名称', trigger: 'blur' }
    ],
    pageSecret: [
        {
            required: true,
            message: '请填写page sccret',
            trigger: 'blur',
        }
    ],
    pageId: [
        {
            required: true,
            message: '请填写 page id',
            trigger: 'blur',
        }
    ]
})

const ruleForm = reactive({
    pageSyncType: 'page',
    pageTitle: '',
    pageSecret: '',
    pageId: '',
    uuId: ''
})
watchEffect(() => {
    // 初始化
    ruleForm.pageSyncType = props.pageSyncType
    ruleForm.pageTitle = props.pageTitle
    ruleForm.pageSecret = props.pageSecret
    ruleForm.pageId = props.pageId
    ruleForm.uuId = props.uuId
})


// 保存
const submitForm = (formEl) => {
    if (!formEl) return
    formEl.validate((valid) => {
        if (valid) {
            emit('updateConfig', { ...ruleForm })
        }
    })
}
// 重置
const deleteForm = () => {
    emit('deleteConfig', { ...ruleForm })
}
const connectPage = (formEl) => {
    // 清空操作
    pageDropList.value = []
    if (!formEl) return
    formEl.validateField('pageSyncType', (pageSyncTypeValid) => {
        if (pageSyncTypeValid) {
            formEl.validateField('pageSecret', async (pageSecretValid) => {
                if (pageSecretValid) {
                    const loading = ElLoading.service({
                        lock: true,
                        text: "获取页面中...",
                        background: "rgba(0, 0, 0, 0.7)",
                    });
                    try {
                        const responseData = await searchNotion(ruleForm.pageSyncType, ruleForm.pageSecret);
                        if (responseData && responseData.results && responseData.results.length > 0) {
                            if (ruleForm.pageSyncType === 'page') {
                                for (const index in responseData.results) {
                                    const result = responseData.results[index]
                                    const type = result.parent.type
                                    let plainTitle = ''
                                    if (type != 'database_id') {
                                        plainTitle = result?.properties?.title?.title[0]?.plain_text | ''
                                    } else {
                                        plainTitle = result?.properties?.Name?.title[0]?.plain_text | ''
                                    }
                                    pageDropList.value.push({
                                        pageId: result.id,
                                        plainTitle: plainTitle
                                    })

                                }
                            } else if (ruleForm.pageSyncType === 'database') {
                                for (const index in responseData.results) {
                                    const result = responseData.results[index]
                                    pageDropList.value.push({
                                        pageId: result.id,
                                        plainTitle: result?.title[0]?.plain_text | ''
                                    })
                                }
                            }
                        }
                        isShowDropNotion.value = true
                    } finally {
                        loading.close();
                    }

                }
            })
        }
    })

}
const handleCommand = (commandValue) => {
    ruleForm.pageId = commandValue
}
/// 情况pageId
const changeSelected = () => {
    ruleForm.pageId = ''
}

</script>

<style scoped>
.setting-form {
    width: 800px;
    border-radius: 14px;
    padding: 18px;
    background-color: rgb(224, 224, 224);
    margin-top: 20px;
}

.setting-form-color {
    background-color: rgb(211, 231, 156) !important;
}

.setting-form-header {
    display: flex;
    align-items: center;
}

.form-header-title {
    margin-left: 10px;
    font-size: 21px;
    font-weight: bold;
}

.el-form {
    margin-top: 20px;
}

.el-dropdown-button {
    display: flex;
    justify-content: center;
    align-items: center;
    flex-direction: row;
    background-color: rgb(203, 198, 142);
    height: 30px;
    margin-left: 20px;
    border-radius: 25px;
    width: 120px;
}

.el-dropdown-link :hover {
    border-color: rgb(203, 198, 142);
}

.el-dropdown-link {
    cursor: pointer;
    display: flex;
    align-items: center;
}
</style>